...
Максимум 5-6 абзацев суммарно.""",
    "block_mission": """АКЦЕНТ: призвание и дело жизни.

Смотри на ВСЮ карту, но начни с этих точек:

HD — АКЦЕНТЫ:
• Крест воплощения (4 ворот Солнца/Земли, сознательные и бессознательные) — это ось темы жизни. Используй описания из секции КРЕСТ ВОПЛОЩЕНИЯ. Синтезируй 4 ворот в одну суть о том, зачем этот человек здесь.
• Тип и стратегия — как реализуется энергия. Проектор ждёт приглашения и видит систему. Генератор реализует через отклик тела. Это меняет всё про карьеру.
• Каналы и ключевые ворота — конкретные таланты, которые встроены в конструкцию.

АСТРОЛОГИЯ — АКЦЕНТЫ:
• MC (Середина Неба) и планеты в 10-м доме — что стоит на вершине карты, что видит мир
• Северный Узел (знак и дом) — куда тянет, что не даётся легко, но именно это и есть путь
• Солнце (дом) — где разворачивается идентичность, где Аполлон хочет сиять
• Юпитер (дом) — где расцветает, где удача и расширение

СИНТЕЗ (главное):
Найди где HD и астрология говорят одно — это факт про человека. Найди напряжение между тем что он хочет (сознательное Солнце) и тем что должен (бессознательное Солнце дизайна). Назови конкретные сферы работы, формат (один/команда, создаёт/передаёт, ведёт/поддерживает). Что нужно для реализации — условие среды, признания, свободы.

Используй ВСЮ карту: если в ней есть что-то важное про призвание вне акцентов — говори об этом.

ЗАПРЕТ: никаких номеров ворот, домов, знаков зодиака в тексте. Только суть через богов и поведение.
...
АСТРОЛОГИЯ — АКЦЕНТЫ:
• 7-й дом (знак куспида, планеты) — образ партнёра, тип связи которую притягивает
• Правитель 7-го дома (его дом) — где и как встречает настоящее
• Венера (дом) — как любит, что нужно чтобы раскрыться
• Марс (дом) — как желает, как конфликтует, что возбуждает
• 5-й дом — романтика и флирт; 8-й дом — глубина и слияние
• Лилит — теневой паттерн: что притягивает как магнит и потом пугает
• Северный Узел (дом) — куда зовёт душа в теме отношений

HD — АКЦЕНТЫ:
• Ворота любви (из секции ВОРОТА ЛЮБВИ) — какой тип любви: мундан (личная) или антимундан (любящий всех)?
• Авторитет — как принимать решения об отношениях
• Открытые центры — где притягивает людей, где есть уязвимость и идеализация

СИНТЕЗ (главное):
Найди главный паттерн который идёт и через астрологию и через HD. Назови точно: что этот человек делает в отношениях, что разрушает близость (один конкретный паттерн, не список). Что нужно от партнёра, но никогда не просится прямо. Какой тип партнёра подходит — конкретный характер.

Используй ВСЮ карту: если есть важные аспекты между планетами отношений — говори.

ЗАПРЕТ: никаких знаков зодиака и номеров домов в тексте. Только суть через богов и поведение.
...
АСТРОЛОГИЯ — АКЦЕНТЫ:
• 2-й дом (знак, планеты) — личные ресурсы, ценности, как зарабатывает
• Правитель 2-го дома (его дом) — путь к деньгам
• 8-й дом — чужие деньги, трансформация ресурсов, скрытые источники
• Юпитер (дом) — где расширяется, где удача; Сатурн (дом) — где ограничение и структура
• Плутон — трансформация финансовых паттернов

HD — АКЦЕНТЫ:
• Эго-центр (определён/открыт) — воля, договоры, самооценка
• Сакральный центр — рабочий ресурс и энергия для дела
• Каналы и ворота связанные с материей (ворота 2, 14, 21, 40, 44, 54)

СИНТЕЗ:
Начни с одной фразы — что сейчас происходит у богов вокруг темы денег (игриво, образно).
Потом — четыре конкретных механизма:
1. Как включается поток — через что именно (усилие, связи, глубину, признание)
2. Главный блок — точный паттерн поведения когда деньги не идут
3. Что хронически занижается и почему — через богов, конкретно
4. Когда деньги идут легко — что при этом совпадает в жизни

ЗАПРЕТ: никаких знаков зодиака кроме вступительной фразы. Никакого вопроса в конце.""",

    "block_health": """АКЦЕНТ: энергия, тело, восстановление.

Смотри на ВСЮ карту, но начни с этих точек:

HD — АКЦЕНТЫ:
• ПЕРЕМЕННЫЕ (4 стрелки) — это самое точное про восстановление и среду. Используй данные из секции ПЕРЕМЕННЫЕ:
  - Голова (детерминация/питание): как устроено отношение к телу и еде
  - Аджна (среда): нужна активная меняющаяся среда или постоянная и уютная
  - Горло (мотивация): что движет изнутри, от страха потери или от надежды
  - G-центр (когниция): как воспринимает информацию — логикой или инсайтами
• Тип — сколько энергии есть по природе и как она работает
• Определённые/открытые центры — откуда берёт энергию и где теряет

АСТРОЛОГИЯ — АКЦЕНТЫ:
• Луна (знак и дом) — эмоциональное тело, что нужно для внутреннего покоя
• 6-й дом (знак, планеты) — здоровье, рутина, способность служить без потери себя
• 12-й дом — что нужно для восстановления (уединение, сон, созерцание)
• Сатурн (дом) — где хроническое ограничение, откуда идёт тревога тела
• Нептун — растворение границ, риск истощения через слияние

СИНТЕЗ:
Найди главное: как этот человек теряет энергию быстрее всего (конкретные ситуации и люди). Что реально восстанавливает — по природе, не по совету. Что он называет ленью а это необходимость. Как переменные дают ему конкретную карту среды и режима.

Используй ВСЮ карту: всё что говорит про тело, ресурс, нервную систему — важно.

ЗАПРЕТ: никаких знаков зодиака и терминов HD в тексте. Только суть через богов и конкретные описания.
