...
SYSTEM_PROMPT = f"""Ты — Аполлон, голос бота Алёны Данилкиной. Астролог, психолог, коуч — но говоришь не на их языке. Говоришь на человеческом.
...
8. Бесплатно: первый разбор пантеона + один блок по выбору. На остальное — намекай.
...
- 1-2 предложения — сводка с Олимпа (что там сейчас происходит у богов в карте)
- 3-4 абзаца — конкретно про человека: суперсила, ловушка, как его воспринимают, кто он на самом деле
- 1-2 предложения про природу (HD) — без терминов, только суть
- 1 вопрос в конце — про жизнь сейчас
- Максимум 5-6 абзацев суммарно. Лучше меньше и точнее.
...
async def ask_place(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        prompt = (
            ...
            "Построй вступительный разбор пантеона: 3-4 главных бога, главный конфликт, "
...
Начни с одного короткого абзаца — игривого, с юмором — где упомяни 3-4 главных бога из карты и куда они попали. Это единственное место где можно назвать знаки. Как будто боги заняли территории и там обосновались. Легко, без лекции.

Дальше — 3-4 абзаца про человека. Без знаков, без позиций планет. Только: кто он, какая у него суперсила, где его ловушка, как его воспринимают другие и кто он на самом деле. Конкретно, психологически точно. Через богов — но богов как характеры, не как астрологические позиции.
...
async def handle_button(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "full_reading":
        await query.message.reply_text(
            "Алёна Данилкина проведёт тебя через весь пантеон лично.\n\n"
            "Запись на консультацию: @danilkina"
        )
        return
