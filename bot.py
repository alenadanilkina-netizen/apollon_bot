...
BLOCK_PROMPTS = {
    "block_identity": """АКЦЕНТ: характер и таланты.

Смотри на ВСЮ карту — астрологию и HD — и дай человеку портрет самого себя.

Начни с одного короткого абзаца — живого, с иронией — где упомяни 3-4 главных бога из карты. ТОЛЬКО ЗДЕСЬ можно назвать знаки и позиции. Как будто боги заняли территории и там обосновались.

Дальше — синтез из обеих карт:

АСТРОЛОГИЯ — АКЦЕНТЫ:
• Солнце (дом) — кто он по сути, где хочет сиять
• Луна (дом) — эмоциональная природа, что нужно для ощущения себя
• Асцендент — как его воспринимают, первое впечатление vs реальность
• Доминирующие планеты и самые сильные аспекты — что управляет жизнью

HD — АКЦЕНТЫ:
• Тип и стратегия — природа энергии и способ действия
• Профиль (линии) — роль, которую он играет в жизни
• Определённые центры — постоянные качества, на что всегда можно опереться
• Ключевые каналы — таланты встроенные в конструкцию

СИНТЕЗ (главное):
Найди где астрология и HD говорят одно — это факт о человеке. Назови суперсилу конкретно (не "ты чувствуешь глубже"). Назови ловушку конкретно — поведение которое человек повторяет и которое ему дорого стоит. Как его воспринимают другие vs кто он на самом деле.

ЗАПРЕТ: никаких знаков зодиака и позиций планет после первого абзаца. Только характер и поведение.
Максимум 5-6 абзацев. Один точный вопрос в конце.""",

    "block_mission": """АКЦЕНТ: предназначение.

Смотри на ВСЮ карту. Предназначение — это не профессия. Это то зачем человек здесь, его тема жизни и способ реализации.

HD — АКЦЕНТЫ:
• Крест воплощения — 4 ворот (Солнце/Земля сознательные + бессознательные). Используй описания из секции КРЕСТ ВОПЛОЩЕНИЯ. Сознательная ось = что хочет. Бессознательная = что должен, даже не понимая почему. Синтезируй в тему жизни — не называя номера ворот.
• Тип + стратегия — механика реализации. Проектор реализуется через признание и приглашение. Генератор — через отклик тела. Это меняет всё.
• Профиль — роль: исследователь, мученик, оппортунист, еретик, ролевая модель — как это проявляется конкретно.
• Каналы — встроенные таланты которые уже активны.

АСТРОЛОГИЯ — АКЦЕНТЫ:
• MC + планеты в 10м доме — что мир видит в этом человеке, публичная роль
• Северный Узел (дом) — направление роста. Это некомфортно, сопротивляется — и именно туда нужно.
• Солнце (дом) — где разворачивается идентичность
• Юпитер (дом) — где расцветает, где удача

СИНТЕЗ (главное):
Где HD и астрология говорят одно — это факт, говори уверенно. Конкретные сферы деятельности (называй: психолог, преподаватель, творческий директор, консультант и т.д.). Формат: с людьми или без, создаёт или передаёт, ведёт или поддерживает. Главное условие для реализации.

Используй ВСЮ карту: всё что говорит о пути и смысле.
...
Смотри на ВСЮ карту. Отношения — это и то как человек любит, и то что притягивает, и то что разрушает.

АСТРОЛОГИЯ — АКЦЕНТЫ:
• 7й дом (знак куспида + планеты) — образ партнёра которого притягивает
• Правитель 7го дома (его дом) — как и где встречает настоящее
• Венера (дом) — как любит, что нужно чтобы раскрыться в отношениях
• Марс (дом) — как желает, как конфликтует
• 5й дом — романтика и флирт; 8й дом — глубина, слияние, трансформация через партнёра
• Лилит — теневой паттерн: что притягивает как магнит и потом пугает
• Северный Узел (дом) — куда зовёт душа в теме близости

HD — АКЦЕНТЫ:
• Ворота любви (из секции ВОРОТА ЛЮБВИ) — мундан (личная) или антимундан (любит всех)
• Авторитет — как принимать решения об отношениях правильно
• Открытые центры — где уязвимость и идеализация партнёра

СИНТЕЗ (главное):
Главный паттерн через обе системы. Точно: что этот человек делает в отношениях, что разрушает близость — один паттерн, без списка. Что нужно от партнёра но никогда не просится напрямую. Какой тип партнёра подходит — конкретный характер, не список качеств.

Используй ВСЮ карту: важные аспекты между Венерой, Марсом, Луной — говори о них.

ЗАПРЕТ: никаких знаков зодиака и номеров домов после первого абзаца. Только поведение и характер.
Максимум 6 абзацев. Один точный вопрос в конце.""",

    "block_money": """АКЦЕНТ: деньги и материальный поток.

Смотри на ВСЮ карту. Деньги — это не только 2й дом. Это про ценности, силу, самооценку и страхи.

АСТРОЛОГИЯ — АКЦЕНТЫ:
• 2й дом (знак + планеты) — личные ресурсы, ценности, как зарабатывает
• Правитель 2го дома (его дом) — путь к деньгам
• 8й дом — чужие деньги, трансформация ресурсов, скрытые источники
• Юпитер (дом) — где расширяется и везёт; Сатурн (дом) — где ограничение и долгий путь
• Плутон — трансформация финансовых паттернов, власть и деньги

HD — АКЦЕНТЫ:
• Эго-центр (определён/открыт) — воля, договоры, самооценка в деньгах
• Сакральный центр — рабочий ресурс: есть постоянная энергия или нет
• Каналы и ворота связанные с материей и ценностью

СИНТЕЗ:
Начни с одной фразы — что происходит у богов вокруг темы денег (игриво, с иронией).
Потом четыре конкретных механизма:
1. Как включается поток — через что именно (усилие, связи, глубину, признание, скорость)
2. Главный блок — точный паттерн поведения когда деньги не идут
3. Что хронически занижается и почему — конкретно, через богов
4. Когда деньги идут легко — что при этом совпадает в жизни

Используй ВСЮ карту: что ещё говорит о деньгах и ценности.

ЗАПРЕТ: никаких знаков зодиака после первой фразы. Никакого вопроса в конце.""",

    "block_health": """АКЦЕНТ: здоровье и тело.

Смотри на ВСЮ карту. Здоровье — это и физическое тело, и нервная система, и хронические паттерны.

АСТРОЛОГИЯ — АКЦЕНТЫ:
• 6й дом (знак + планеты) — здоровье, рутина, тело как инструмент
• Правитель 6го дома (его дом) — откуда приходят сложности
• Луна (дом) — эмоциональное тело: что разрушает психосоматически
• Сатурн (дом) — хронические зоны уязвимости, где нужна структура
• Плутон — где идут глубокие телесные трансформации
• 12й дом — скрытые болезни, психосоматика, что тело хранит в тени

HD — АКЦЕНТЫ:
• Тип — сколько энергии по природе. Проектор: нет постоянной энергии Генератора, это факт
• Определённые центры — стабильные зоны; Открытые — где тело впитывает чужое и устаёт
• Селезёнка (определена/открыта) — инстинктивный иммунитет, страхи тела
• Корень (определён/открыт) — давление, стресс, адреналин как норма жизни

СИНТЕЗ:
Где тело даёт сигналы что что-то идёт не так — конкретные симптомы или паттерны. Хроническое напряжение: откуда оно берётся и как проявляется. Что этот человек делает с телом что ему вредит — точный паттерн. Что было бы правильно изменить в отношении к телу — конкретно.

Используй ВСЮ карту.

ЗАПРЕТ: никаких знаков зодиака и терминов. Только суть через богов и поведение.
Максимум 5 абзацев. Один точный вопрос в конце.""",

    "block_resources": """АКЦЕНТ: ресурсы — энергия, среда, восстановление.

Смотри на ВСЮ карту. Ресурсы — это про то как человек заряжается и разряжается, какая среда питает, как принимать правильные решения о силе.

HD — ГЛАВНЫЙ ИСТОЧНИК:
• ПЕРЕМЕННЫЕ (4 стрелки) — из секции PHS ПЕРЕМЕННЫЕ. Это самая точная карта восстановления:
  - ДЕТЕРМИНАЦИЯ (Personality Sun) — тип питания и отношения к телу (Звук/Свет/Вкус/Касание и т.д.)
  - СРЕДА (Design Node) — оптимальное окружение (Пещера/Рынок/Кухня/Гора/Долина/Берег)
  - МОТИВАЦИЯ (Personality Earth) — что реально движет, а не то что человек думает
  - КОГНИЦИЯ (Design Earth) — как воспринимает и обрабатывает информацию
• Тип — Проектор не имеет постоянной сакральной энергии. Это не слабость — это другая механика: работа циклами, нужен отдых до исчерпания.
• Открытые центры — где идёт утечка, что забирают другие люди
• Авторитет — как принимать решения о нагрузке и среде

АСТРОЛОГИЯ — АКЦЕНТЫ:
• Луна (дом) — что эмоционально восстанавливает, что разрушает
• 12й дом — природное пространство для уединения и восстановления
• Сатурн — где нужна структура чтобы не сжечь себя
• Нептун — риск растворения и истощения через слияние с другими

СИНТЕЗ:
Как этот человек теряет энергию быстрее всего — конкретные ситуации, типы людей, форматы. Что реально восстанавливает по природе — не по совету. Что он называет ленью а это необходимость. Конкретная среда и режим которые подходят именно этой природе.

Используй ВСЮ карту.

ЗАПРЕТ: никаких знаков зодиака и терминов HD в тексте.
Максимум 5 абзацев. Один точный вопрос в конце.""",
}
