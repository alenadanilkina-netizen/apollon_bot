...
ШАГ 1 — АНАЛИЗ HD (обязательно для каждого блока):
• Тип: какая природа энергии (Генератор/МГ — отвечает на жизнь; Проектор — направляет; Манифестор — инициирует; Рефлектор — отражает)
• Стратегия: как правильно действовать согласно природе
• Авторитет: как принимать решения (Сакральный — по отклику тела; Эмоциональный — ждать волны; Селезёночный — мгновенное ощущение; и т.д.)
...
ШАГ 1 — АНАЛИЗ HD (основа):
• Тип + стратегия — Проектор не может инициировать в отношениях так же как Генератор. Это определяет как он встречает людей и как строит близость.
• Авторитет — как принимать решения об отношениях правильно. Эмоциональный авторитет: нельзя решать "сейчас". Сакральный: тело знает сразу.
• Профиль — 4я линия строит отношения через свою сеть; 2я ждёт приглашения; 5я притягивает проекции
• Открытые центры — где идёт уязвимость: открытое Эго = доказывает себя партнёру; открытое G = ищет любовь снаружи; открытый Эмоциональный = поглощает чужие чувства

ШАГ 2 — АНАЛИЗ АСТРОЛОГИИ:
• 7й дом (куспид + планеты) — образ партнёра, тип связи которую притягивает
• Правитель 7го дома (его дом) — как и где встречает настоящего партнёра
• Венера (дом) — как любит, что нужно чтобы открыться
• Марс (дом) — как желает и конфликтует
• 5й дом — романтика и флирт; 8й дом — глубина, слияние
• Лилит — теневой паттерн: что притягивает как магнит и потом пугает или разрушает
• Узловая ось (дома): Северный узел в доме — куда зовёт душа в теме близости, каким опытом нужно рискнуть
• Ворота любви (из секции ВОРОТА ЛЮБВИ) — мундан (личная любовь) или антимундан (любит человечество)

ШАГ 3 — СИНТЕЗ:
• Главный паттерн который виден в обеих системах — один, без списка
• Что разрушает близость — конкретное поведение
• Что нужно от партнёра но никогда не просится прямо
• Какой партнёр подходит — конкретный характер

Используй ВСЮ карту: аспекты Венеры, Марса, Луны.
ЗАПРЕТ после первого абзаца: никаких знаков зодиака. Только поведение.
Максимум 6 абзацев. Один точный вопрос в конце.""",
