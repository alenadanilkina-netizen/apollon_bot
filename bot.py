...
5. Имена богов — только в шутке в начале и в конце. В основном тексте не называй планеты, их положения, достоинства или экзальтации: переводи расчёт в наблюдаемую ситуацию, выбор или способ действовать. Не «Афродита у тебя...», «Арес решил...» и не «Венера в экзальтации».
...
def ready_block_reading(uid: int, block: str) -> str:
    ...
    lens_by_block = {
        "block_identity": f"{type_hint}\n\n{decision_hint}",
        "block_mission": f"{type_hint}\n\nТвой способ выбирать направление важнее чужого представления о скорости успеха.",
        "block_potential": decision_hint,
        "block_love": "В близости особенно важно не торопить решение о доверии: сначала замечай поступки, затем делай вывод о взаимности.",
        "block_money": "Финансовые решения полезно принимать после паузы и проверки условий: спешка чаще всего прячет не выгоду, а неясность.",
        "block_health": "Нагрузка становится бережнее, когда решения о темпе принимаются не из тревоги и не из чувства долга, а с учётом реального ресурса.",
        "block_resources": f"{type_hint}\n\nВосстановление легче строить вокруг своего естественного темпа, а не вокруг чужой дисциплины.",
    }
    lens = lens_by_block.get(block, decision_hint)
    return (
...
6. Не называй планеты, знаки, дома, аспекты, достоинства и экзальтации в пользовательском тексте. Переводи расчёт в действие, выбор, реакцию или ситуацию.
...
_PUBLIC_TECHNICAL_LEAK_RE = re.compile(
    r"(?:\bворот\w*\s*[-—–]?\s*\d{1,2}\b|\b\d{1,2}(?:[-‑–—]?(?:е|х))?\s+ворот\w*|"
        ...
        r"дизайн\w*\s+человек|экзальтац\w*|планет\w*\s+в\s+(?:ущербе|падении)|"
        r"натальн\w*|транзит\w*|соляр\w*|лунар\w*|прогресси\w*|"
        r"аспект\w*|квадрат\w*|тригон\w*|оппозици\w*|секстил\w*|"
        r"ретроград\w*|активаци\w*|гексаграмм\w*|\b(?:асцендент|десцендент|мс|ас)\b)\b)",
    re.IGNORECASE,
)
...
def get_forecast_prompt(period: str, transits_data: str) -> str:
    ...
    return f"""Сделай понятный прогноз про {labels.get(period, 'этот период')}.

Расчётные данные ниже — только твой внутренний материал. Пользователь не
должен увидеть ни названий систем, ни названий небесных тел, ни знаков, ни
домов, ни аспектов, ни линий, ворот, каналов, центров, циклов или прогрессий.
Не объясняй, по какой технике собран ответ. Смысл важнее аппаратуры.

ФОРМАТ ОТВЕТА:

1. Короткий заголовок, который называет человеческий процесс: например,
«Проверка старых договорённостей».

2. Один абзац: когда начинается и заканчивается период, если границы реально
есть в данных. Для одного дня честно назови это снимком дня, не обещанием на
весь месяц.

3. Два–три абзаца о том, что стоит заметить в делах и деньгах, отношениях и
внутреннем состоянии. Каждую мысль связывай с наблюдаемой ситуацией: разговор,
срок, выбор, договорённость, граница, нагрузка. Не повторяй одну мысль в трёх
разделах.

4. Один конкретный шаг, который можно проверить в этом периоде. Заверши одной
короткой иронией Олимпа. Это символическая интерпретация, не гарантия события.

ДОПОЛНИТЕЛЬНО ДЛЯ МЕСЯЦА:
— Если данные дают две последовательные границы, раздели текст на «до [дата]»
и «после [дата]». Не называй расчётную технику, которая дала границу.
— Если расчёт сделан по месту рождения, скажи по-человечески: «для точности
картины полезно указать город, где ты сейчас живёшь».

ДОПОЛНИТЕЛЬНО ДЛЯ ГОДА:
— Раздели год на временные части только там, где в данных есть реальная смена
фокуса. Не обещай событий и не называй расчётные причины этих смен.

ОБЯЗАТЕЛЬНЫЕ ОГРАНИЧЕНИЯ:
— Не придумывай даты, факты и поворотные события.
— Не используй слова: «натальный», «транзит», «соляр», «лунар», «прогрессия»,
«аспект», «квадрат», «трин», «оппозиция», «секстиль», «ретроградный», «ворота»,
«линия», «канал», «центр», «бодиграф», «Дизайн Человека».
— Не начинай абзацы словами «это значит» и не используй «возможно» вместо
