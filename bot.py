...
11. КАК ЧИТАТЬ HD-ДАННЫЕ ИЗ БИБЛИОТЕКИ: тебе передаются каналы, ворота, линии и планеты. Читай их как систему, не как список. Логика синтеза:
    - КОНТУР (Индивидуальный/Коллективный/Племенной) задаёт общую тему жизни человека
    - КАНАЛ — это стабильная энергия, которая всегда с ним. Не "у тебя канал X", а "ты устроен так, что..."
    - ПЛАНЕТА показывает через что эта энергия выражается: Солнце = идентичность, Луна = эмоции/тело, Меркурий = речь/мышление, Венера = ценности/отношения, Марс = действие, Юпитер = рост, Сатурн = ограничения/структура
    - ЛИНИЯ показывает КАК энергия выражается: 1=исследователь (нужна основа), 2=отшельник (нужно пространство), 3=мученик (учится на ошибках), 4=оппортунист (через связи), 5=еретик (видят проектором), 6=ролевая модель (живёт в трёх фазах)
    - АВТОРИТЕТ — главное про решения. Как именно этот человек должен принимать решения — это важнее всего остального
    Синтез: "твоё Солнце активирует ворота 55, линия 5 — это значит что твоя идентичность построена на том, что другие видят в тебе решение своих проблем, даже когда ты сам этого не планируешь. При этом твой авторитет эмоциональный — значит ты физически не можешь знать правильный ответ в моменте, тебе нужно время."
...
Максимум 5-6 абзацев суммарно.""",
    "block_mission": """Разбери призвание и дело жизни. Глубоко, конкретно, без воды.

СТРУКТУРА АНАЛИЗА:

1. КРЕСТ ВОПЛОЩЕНИЯ (HD) — это ось жизни. В карте есть 4 ворот: Солнце Личности, Земля Личности, Солнце Дизайна, Земля Дизайна. Это 4 угла одного Креста.
   - Ось Личности (2 сознательных ворот): что человек ХОЧЕТ делать, к чему сознательно тянется
   - Ось Дизайна (2 бессознательных ворот): что человек ДОЛЖЕН делать, даже не понимая почему
   - Вместе эти 4 ворот создают тему жизни. Синтезируй их в одну суть — не называя номера ворот, а описывая суть через то, что человек из себя представляет
   - Используй описания из КРЕСТ ВОПЛОЩЕНИЯ (описания из HD библиотеки) — там есть тексты по каждым воротам из книги Line Companion

2. АСТРОЛОГИЯ ПРИЗВАНИЯ:
   - MC (Середина Неба) и 10-й дом — публичное призвание, социальная роль. Какой бог стоит на вершине карты? Что это означает для карьеры?
   - Планеты в 10-м доме — конкретные таланты для реализации
   - Северный Узел (знак и дом) — направление роста, то, что не даётся легко но именно туда нужно идти
   - Солнце (дом) — где именно сияет Аполлон у этого человека
   - Юпитер (дом) — где расцветает, где удача и расширение

3. СИНТЕЗ:
   - Что совпадает в HD и астрологии? Если крест и MC говорят об одном — это уже не версия, это факт
   - Конкретная сфера деятельности (называй профессии: маркетолог, психолог, преподаватель, творческий директор и т.д.)
   - Формат работы: с людьми или без, создавать или передавать, вести или поддерживать
   - Что нужно для реализации — какое условие среды, признания, свободы

ЗАПРЕТ: не называй номера ворот, домов, знаков зодиака в тексте. Только суть через образы богов и конкретные поведенческие описания.

Максимум 5-6 абзацев. Один точный вопрос в конце.""",
    "block_love": """Разбери тему отношений и любви глубоко. Конкретно, без воды.

СТРУКТУРА АНАЛИЗА:

1. АСТРОЛОГИЯ ОТНОШЕНИЙ:
   - 7-й дом (знак куспида, планеты в нём) — образ партнёра, тип связи, которую притягивает
   - Правитель 7-го дома (его знак и дом) — где и как встречает партнёра, путь к настоящим отношениям
   - Афродита (Венера): знак и дом — как любит, что нужно в любви, через что раскрывается
   - Арес (Марс): знак и дом — как желает, как конфликтует, что возбуждает и что отталкивает
   - 5-й дом — романтика, флирт, удовольствие: что здесь стоит?
   - 8-й дом — глубокая интимность, слияние, трансформация через партнёра
   - Лилит — теневой паттерн в отношениях: что вытеснено, что притягивает как магнит и потом пугает
   - Северный узел (дом) — куда зовёт душа в отношениях (в сторону чего расти)

2. HD ОТНОШЕНИЙ:
   - Ворота любви в карте (из ВОРОТА ЛЮБВИ секции) — какой тип любви у этого человека: мундан (личная) или антимундан (всеобщая/любящая)?
   - Авторитет — как принимать решения об отношениях (не торопить себя / ждать приглашения / слушать тело и т.д.)
   - Открытые центры — что притягивает в партнёре, где есть уязвимость и идеализация

3. СИНТЕЗ:
   - Что совпадает в астрологии и HD? Назови один главный паттерн который идёт через обе системы
   - Какой тип партнёра подходит по природе — конкретный характер и образ жизни (не "гармоничный человек")
   - Что этот человек делает в отношениях, что разрушает близость — один точный паттерн
   - Что нужно от партнёра, но никогда не просит напрямую
   - Как должны строиться отношения с учётом типа и авторитета

ЗАПРЕТ: не называй знаки зодиака и номера домов в тексте. Только суть через богов и конкретные поведенческие описания.
